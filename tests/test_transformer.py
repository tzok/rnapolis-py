...
def test_replace_value():
    ...
    org_label_asym_id = set(
        [atom.label.chain for atom in org_atoms if atom.label is not None]
    )
    org_auth_asym_id = set([atom.auth.chain for atom in org_atoms])
    ...
    rep_label_asym_id = set(
        [atom.label.chain for atom in rep_atoms if atom.label is not None]
    )
    rep_auth_asym_id = set([atom.auth.chain for atom in rep_atoms])
