...
import pytest
import pandas as pd
from rnapolis.parser_v2 import parse_pdb_atoms, parse_cif_atoms
...
def test_parse_4qln_formats(data_dir):
    ...
    pdb_residue_ids = [(r.chain_id, r.residue_number, r.insertion_code) for r in pdb_residues]
    cif_residue_ids = [(r.chain_id, r.residue_number, r.insertion_code) for r in cif_residues]
    ...
    for i, (pdb_id, cif_id) in enumerate(zip(pdb_residue_ids, cif_residue_ids)):
        assert pdb_id == cif_id, f"Residue mismatch at position {i}: PDB={pdb_id}, mmCIF={cif_id}"
    
    # Compare residue names
    pdb_residue_names = [r.residue_name for r in pdb_residues]
    cif_residue_names = [r.residue_name for r in cif_residues]
    
    # Create a mapping from residue ID to residue name for both formats
    pdb_id_to_name = {(r.chain_id, r.residue_number, r.insertion_code): r.residue_name 
                      for r in pdb_residues}
    cif_id_to_name = {(r.chain_id, r.residue_number, r.insertion_code): r.residue_name 
